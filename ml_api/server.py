...
from lib.detection_model_v2 import image_detection
import lib.darknet as darknet
...
def get_p():
    if 'img' in request.args:
        try:
            ...
            _, detections = image_detection(image, network, class_names, class_colors, THRESH)
            app.logger.debug(f"{len(detections)} detections")
            print(detections)
